...
class ArticleCateModel extends Model {
    ...
    public function get_cate_list(){
        //获取文章父级分类
        $cate_list = $this->selectArticleCate(array('parent_id'=>0),'sort_order desc');
        foreach ($cate_list as $key => $value) {
            //获取子分类
            $cate_list[$key]['cate_child_list'] = $this->selectArticleCate(array('parent_id'=>$value['cate_id']),'');
        }
        return $cate_list;
    }

    /**
     *添加下拉选择菜单时 2014-12-19
     * $parent_id 父级ID
     * $is_child 是否显示子级
     */
