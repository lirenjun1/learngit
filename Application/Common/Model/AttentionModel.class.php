...
class AttentionModel extends Model {
     ...
     * 查询多条数据
     */
        public function selectAttention($where = array(),$order = '',$page_size = '',$parameter = array()){
     ...
     * 添加数据
     */
    public function addAttention($data){
        if(empty($data)){
            return false;
        }else{
     ...
     * 查询一条数据
     */
    public function findAttention($where){
        if(empty($where)){
            return false;
        }else{
            if($where['status'] == '' || empty($where['status'])){
     ...
     * 编辑数据
     */
    public function editAttention($where,$data){
     ...
     * 删除数据
     */
    public function deleteAttention($where){
        if(empty($where)){
            return false;
        }
