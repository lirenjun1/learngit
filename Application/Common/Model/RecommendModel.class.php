...
class RecommendModel extends Model {
     ...
     * 查询多条数据
     */
        public function selectRecommend($where = array(),$order = '',$page_size = '',$parameter = array()){
     ...
     * 查询一条数据
     */
    public function findRecommend($where){
        if(empty($where)){
            return false;
        }else{
            if($where['status'] == '' || empty($where['status'])){
     ...
     * 编辑数据
     */
    public function editRecommend($where,$data){
     ...
     * 删除数据
     */
    public function deleteRecommend($where){
        if(empty($where)){
            return false;
        }
