...
class CollectionModel extends Model {
     ...
     * 查询多条数据
     */
        public function selectCollection($where = array(),$order = '',$page_size = '',$parameter = array()){
     ...
     * 查询一条数据
     */
    public function findCollection($where){
        if(empty($where)){
            return false;
        }else{
            if($where['status'] == '' || empty($where['status'])){
     ...
     * 删除数据
     */
    public function deleteCollection($where){
        if(empty($where)){
            return false;
        }
